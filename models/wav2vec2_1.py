...
class TransformerEncoder_1(nn.Module):
    ...
    def __init__(self, args):
        ...
        for i in range(args.encoder_layers):
            if i in self.speaker_injection_layers:
                ...
                layers.append(self.build_encoder_layer(args))
    ...
    def forward(self, x, spk_emb, need_weights=False, padding_mask=None, layer=None):
        x, layer_results = self.extract_features(x, spk_emb, need_weights, padding_mask, layer)
        ...
        for i, layer in enumerate(self.layers):
            ...
            if not self.training or (dropout_probability > self.layerdrop) or i in self.speaker_injection_layers:
                if i not in self.speaker_injection_layers:
                    x, (z, lr) = layer(
                        x, self_attn_padding_mask=padding_mask, need_weights=need_weights
                    )
                else:
                    x, (z, lr) = layer(
                        x, spk_emb, self_attn_padding_mask=padding_mask, need_weights=need_weights
                    )
                if i >= min_layer:
