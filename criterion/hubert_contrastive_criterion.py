...
class HubertContrastiveCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True, log_pred=False):
        ...
        logging_output["loss_mlm"] = loss.detach().item()
        ...
        if model.training and net_output['score_list']:
            self_corr_loss = net_output['score_list'][0]
            loss += (self_corr_loss * self.loss_weights[-1])
            logging_output["ctr_loss"] = self_corr_loss.detach().item()

        # add contrastive loss
        # if 'score_list' in net_output and model.training: # score_list is set to [] for eval mode
        ...
        #     weight_ctr = self.loss_weights[-1] * min(model.num_updates, 5000)
        ...
        # attn ctr loss
        # if 'score_list' in net_output and model.training:
        #     loss_ctr = net_output['score_list'][0]
        #     p = self.loss_weights[-1] * loss_ctr
        #     loss += p
        #     logging_output["ctr_loss"] = loss_ctr.detach().item()
        #     logging_output["weighted_ctr_loss"] = p.detach().item()
